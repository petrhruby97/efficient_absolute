...
#include "p2p1ll_modified.cc"
#include "p2p1ll.cc"
#include "p2p1ll_q3q.cc"
#include "p2p1ll_bouaziz.cc"
#include "p2p1ll_lu_bouaziz.cc"

using namespace std::chrono;

void sample_point(const Eigen::Matrix3d R, const Eigen::Vector3d T, Eigen::Vector3d &p, Eigen::Vector3d &P, Eigen::Vector3d &Y0, Eigen::Vector3d &D1, Eigen::Vector3d &D2)
{
        std::normal_distribution<double> norm_sampler(0.0,1.0);
        std::random_device rd;
        std::default_random_engine eng(rd());

        double a1 = norm_sampler(eng);
        double b1 = norm_sampler(eng);
        Eigen::Vector3d X1 = Y0 + a1*D1 + b1*D2;
...
void sample_line(const Eigen::Matrix3d R, const Eigen::Vector3d T, Eigen::Vector3d &d2, Eigen::Vector3d &d3, Eigen::Vector3d &L1, Eigen::Vector3d &L2, Eigen::Vector3d &Y0, Eigen::Vector3d &D1, Eigen::Vector3d &D2)
{
        std::normal_distribution<double> norm_sampler(0.0,1.0);
        std::random_device rd;
        std::default_random_engine eng(rd());
        ...
        double a2 = norm_sampler(eng);
        double b2 = norm_sampler(eng);
        Eigen::Vector3d X2 = Y0 + a2*D1 + b2*D2;
        L2 = X2;
...
void sample(
        ...
        Eigen::Vector3d &d1, Eigen::Vector3d &d2, Eigen::Vector3d &d3, Eigen::Vector3d &d4, //2D points
        Eigen::Vector3d &P1, Eigen::Vector3d &P2, Eigen::Vector3d &L3, Eigen::Vector3d &L4 //3D points
)
{
	...
	//generate the plane on which the point and lines lie
        double x0 = norm_sampler(eng);
        double y0 = norm_sampler(eng);
        double z0 = norm_sampler(eng);
        Eigen::Vector3d Y0(x0, y0, z0+5);

        double x1 = norm_sampler(eng);
        double y1 = norm_sampler(eng);
        double z1 = norm_sampler(eng);
        Eigen::Vector3d Y1(x1, y1, z1+5);
        Eigen::Vector3d D1 = Y1-Y0;

        double x2 = norm_sampler(eng);
        double y2 = norm_sampler(eng);
        double z2 = norm_sampler(eng);
        Eigen::Vector3d Y2(x2, y2, z2+5);
        Eigen::Vector3d D2 = Y2-Y0;


        //generate the correspondences
        sample_point(R,T,d1,P1,Y0,D1,D2);
        sample_point(R,T,d2,P2,Y0,D1,D2);
        sample_line(R,T,d3,d4,L3,L4,Y0,D1,D2);
...
{	
	...
	for(int i=0;i<num_iters;++i)
	{
		...
		//the 3D points
		Eigen::Vector3d P1, P2, L3, L4;
			
		//sample
		sample(Rgt,tgt,d1,d2,d3,d4,P1,P2,L3,L4);
		...
		std::vector<poselib::CameraPose> sols_bouaziz;
		std::vector<poselib::CameraPose> sols_lu_bouaziz;
		std::vector<Eigen::Vector3d> xp(2); //2D point
		xp[0] = d1;
		xp[1] = d2;
		std::vector<Eigen::Vector3d> Xp(2); //3D point
		Xp[0] = P1;
		Xp[1] = P2;
		std::vector<Eigen::Vector3d> l(1); //2D lines
		l[0] = d3.cross(d4);
		std::vector<Eigen::Vector3d> X(1); //point on the 3D line
		X[0] = L3;
		std::vector<Eigen::Vector3d> V(1); //direction of the 3D line
		V[0] = L4-L3;

		//solve
		high_resolution_clock::time_point t1 = high_resolution_clock::now();
		int num_sols_modified = poselib::p2p1ll_modified(xp, Xp, l, X, V, &sols_modified);
		int num_sols = poselib::p2p1ll(xp, Xp, l, X, V, &sols);
		int num_sols_3q3 = poselib::p2p1ll_q3q(xp, Xp, l, X, V, &sols_3q3);
		int num_sols_bouaziz = poselib::p2p1ll_bouaziz(xp, Xp, l, X, V, &sols_bouaziz);
		int num_sols_lu_bouaziz = poselib::p2p1ll_lu_bouaziz(xp, Xp, l, X, V, &sols_lu_bouaziz);
		...
		{
			double rot_err = evaluate_R(sols_modified[j].R(), Rgt);
		...
		{
			double rot_err = evaluate_R(sols[j].R(), Rgt);
		...
		{
			double rot_err = evaluate_R(sols_3q3[j].R(), Rgt);
		...
		double min_err_R_bouaziz = 3.14;
		double min_err_T_bouaziz = 3.14;
		for(int j=0;j<num_sols_bouaziz ;++j)
		{
			double rot_err = evaluate_R(sols_bouaziz[j].R(), Rgt);
			double tran_err = (sols_bouaziz[j].t - tgt).norm()/(sols_bouaziz[j].t.norm());
			if(rot_err < min_err_R_bouaziz )
				min_err_R_bouaziz  = rot_err;
			if(tran_err < min_err_T_bouaziz)
				min_err_T_bouaziz  = tran_err;
		}

		double min_err_R_lu_bouaziz = 3.14;
		double min_err_T_lu_bouaziz = 3.14;
		for(int j=0;j<num_sols_lu_bouaziz ;++j)
		{
			double rot_err = evaluate_R(sols_lu_bouaziz[j].R(), Rgt);
			double tran_err = (sols_lu_bouaziz[j].t - tgt).norm()/(sols_lu_bouaziz[j].t.norm());
			if(rot_err < min_err_R_lu_bouaziz )
				min_err_R_lu_bouaziz  = rot_err;
			if(tran_err < min_err_T_lu_bouaziz)
				min_err_T_lu_bouaziz  = tran_err;
		}
		std::cout << min_err_R_modified << " " << min_err_T_modified << " ";
		std::cout << min_err_R << " " << min_err_T << " ";
		std::cout << min_err_R_3q3 << " " << min_err_T_3q3 << " ";
		std::cout << min_err_R_bouaziz << " " << min_err_T_bouaziz << " ";
		std::cout << min_err_R_lu_bouaziz << " " << min_err_T_lu_bouaziz << "\n";
